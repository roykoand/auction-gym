...
from src.utils.math import normal_pdf
...
class EmpiricalShadedBidder(Bidder):
    ...
    def update(
        ...
        if plot:
            ...
            plt.scatter(gammas, utilities, alpha=0.25)
        ...
        for idx, bucket_hi in enumerate(buckets[1:]):
            ...
            if num_samples > 1:
                ...
                estimated_y_stderr.append(
                    np.std(utilities[mask]) / np.sqrt(num_samples)
                )
            else:
        ...
        if plot:
            ...
            plt.xlabel(
                r"Multiplicative Bid Shading Factor ($\gamma$)", fontsize=fontsize
            )
...
class ValueLearningBidder(Bidder):
    ...
    def bid(self, value, context, estimated_CTR):
        ...
        if not self.model_initialised:
            ...
            x = torch.Tensor(
                np.hstack(
                    (
                        np.tile(estimated_CTR, (n_values_search, 1)),
                        np.tile(value, (n_values_search, 1)),
                        gamma_grid.reshape(-1, 1),
                    )
                )
            )
    ...
    def update(
        ...
        X = np.hstack(
            (
                estimated_CTRs.reshape(-1, 1),
                values.reshape(-1, 1),
                np.array(self.gammas).reshape(-1, 1),
            )
        )
        ...
        optimizer = torch.optim.Adam(
            self.winrate_model.parameters(), lr=lr, weight_decay=1e-6, amsgrad=True
        )
        ...
        print(
            "AUC predicting P(win):\t\t\t\t",
            roc_auc_score(won_mask.astype(np.uint8), W),
        )

        if self.inference == "policy":
            # Learn a policy to maximise E[U | bid] where bid ~ policy
            X = torch.Tensor(
                np.hstack((estimated_CTRs.reshape(-1, 1), values.reshape(-1, 1)))
            )
            ...
            lr = 2e-3
            optimizer = torch.optim.Adam(
                self.bidding_policy.parameters(), lr=lr, weight_decay=1e-6, amsgrad=True
            )
            scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
                optimizer,
                "min",
...
class PolicyLearningBidder(Bidder):
    ...
    def update(
        ...
        lr = 2e-3
        optimizer = torch.optim.Adam(
            self.model.parameters(), lr=lr, weight_decay=1e-4, amsgrad=True
        )
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer,
            "min",
        ...
        for epoch in tqdm(range(int(epochs)), desc=f"{name}"):
            ...
            loss = self.model.loss(
                X, gammas, propensities, utilities, importance_weight_clipping_eps=50.0
            )
        ...
        self.model.eval()
        expected_utility = (
            -self.model.loss(X, gammas, propensities, utilities, KL_weight=0.0)
            .detach()
            .numpy()
        )
...
class DoublyRobustBidder(Bidder):
    ...
    def __init__(self, rng, gamma_sigma, init_gamma=1.0):
        ...
        self.propensities = []
        self.winrate_model = PyTorchWinRateEstimator()
        self.bidding_policy = BidShadingContextualBandit(
            loss="Doubly Robust", winrate_model=self.winrate_model
        )
        self.model_initialised = False
    ...
    def update(
        ...
        ##############################
        gammas_numpy = np.array(
            [g.detach().item() if self.model_initialised else g for g in self.gammas]
        )
        ...
        lr = 7e-3
        optimizer = torch.optim.Adam(
            self.bidding_policy.parameters(), lr=lr, weight_decay=1e-4, amsgrad=True
        )
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer,
            "min",
        ...
        for epoch in tqdm(range(int(epochs)), desc=f"{name}"):
            ...
            loss = self.bidding_policy.loss(
                ...
                propensities,
                utilities,
                utility_estimates=estimated_utilities,
                winrate_model=self.winrate_model,
                importance_weight_clipping_eps=50.0,
            )
            loss.backward()  # Computes the gradient of the given tensor w.r.t. the weights/bias
            optimizer.step()  # Updates weights and biases with the optimizer (SGD)
            losses.append(loss.item())
