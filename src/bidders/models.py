...
class PyTorchWinRateEstimator(torch.nn.Module):
    def __init__(self):
        ...
        self.model = torch.nn.Sequential(
            torch.nn.Linear(3, 1, bias=True), torch.nn.Sigmoid()
        )
        self.eval()
...
class BidShadingPolicy(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        sigma = (
            torch.nn.Softplus()(self.sigma_linear_out(torch.nn.Softplus()(x)))
            + self.min_sigma
        )
...
class BidShadingContextualBandit(torch.nn.Module):
    ...
    def initialise_policy(self, observed_contexts, observed_gammas):
        ...
        optimizer = torch.optim.Adam(
            self.parameters(), lr=lr, weight_decay=1e-4, amsgrad=True
        )
        ...
        for epoch in tqdm(range(int(epochs)), desc=f"Initialising Policy"):
            ...
            predicted_mu_gammas = torch.nn.Softplus()(
                self.mu_linear_out(
                    torch.nn.Softplus()(self.shared_linear(observed_contexts))
                )
            )
            predicted_sigma_gammas = torch.nn.Softplus()(
                self.sigma_linear_out(
                    torch.nn.Softplus()(self.shared_linear(observed_contexts))
                )
            )
            loss = criterion(
                predicted_mu_gammas.squeeze(), observed_gammas
            ) + criterion(
                predicted_sigma_gammas.squeeze(),
                torch.ones_like(observed_gammas) * 0.05,
            )
        ...
        print(
            "Predicted mu Gammas: ",
            predicted_mu_gammas.min(),
            predicted_mu_gammas.max(),
            predicted_mu_gammas.mean(),
        )
        print(
            "Predicted sigma Gammas: ",
            predicted_sigma_gammas.min(),
            predicted_sigma_gammas.max(),
            predicted_sigma_gammas.mean(),
        )
    ...
    def loss(
        ...
        logging_propensity,
        utility,
        utility_estimates=None,
        winrate_model=None,
        KL_weight=5e-2,
        importance_weight_clipping_eps=torch.inf,
    ):
        mean_gamma_target, sigma_gamma_target, target_propensities = self.normal_pdf(
            observed_context, observed_gamma
        )
        ...
        if self.loss_name == "REINFORCE":  # or (not self.model_initialised)
            ...
            KLdiv = (
                sigma_gamma_target**2 + (mean_gamma_target - observed_gamma) ** 2
            ) / (2 * sigma_gamma_target**2) - 0.5
            ...
            importance_weights = target_propensities / logging_propensity
            clipped_importance_weights = torch.clip(
                importance_weights,
                min=1.0 / importance_weight_clipping_eps,
                max=importance_weight_clipping_eps,
            )
            return -torch.min(
                importance_weights * utility, clipped_importance_weights * utility
            ).mean()

        elif self.loss_name == "Doubly Robust":
            importance_weights = target_propensities / logging_propensity

            DR_IPS = (utility - utility_estimates) * torch.clip(
                importance_weights,
                min=1.0 / importance_weight_clipping_eps,
                max=importance_weight_clipping_eps,
            )
            ...
            dist = torch.distributions.normal.Normal(
                mean_gamma_target, sigma_gamma_target
            )

            sampled_gamma = torch.clip(dist.rsample(), min=0.0, max=1.0)
            features_for_p_win = torch.hstack(
                (observed_context, sampled_gamma.reshape(-1, 1))
            )
            ...
            P = (
                observed_context[:, 0].squeeze()
                * observed_context[:, 1].squeeze()
                * sampled_gamma
            )
